// User inputs are stores in controlled form object, using e as event which stores all the <Input> values
// and then we can access them using e.target.value. The form is submitted using the onSubmit event handler,
// The date is updated manually by setformdata function

// Lastly the data is sent to the backend localhost8000 using the predefined WeatherFormData interface
...
export function WeatherForm({ onSuccess }: WeatherFormProps) {
  ...
  const [successfullySubmitted, setSuccessfullySubmitted] = useState(false); // For the celebration animation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setResult(null);
    setSuccessfullySubmitted(false); // reset status
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          setTimeout(() => setSuccessfullySubmitted(false), 2000);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="notes"
              name="notes"
              className="h-48" // Adjust height of the textarea for note
              rows={3}
